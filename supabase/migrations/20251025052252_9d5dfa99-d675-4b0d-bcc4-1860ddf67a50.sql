-- Create enum for programming languages (if not exists)
DO $$ BEGIN
...
DROP POLICY IF EXISTS "Users can view own attempts" ON public.level_attempts;
DROP POLICY IF EXISTS "Users can insert own attempts" ON public.level_attempts;
...
-- Insert default badges (only if not exists)
INSERT INTO public.badges (badge_type, name, description, icon, xp_reward)
VALUES
  ('first_debug', 'First Debug', 'Completed your first debugging challenge', '🐛', 100),
  ('no_hint_pro', 'No Hint Pro', 'Solved a level without using any hints', '🧠', 150),
  ('speed_solver', 'Speed Solver', 'Completed a level in under 2 minutes', '⚡', 200),
  ('streak_killer', '3-Streak Killer', 'Maintained a 3-day streak', '🔥', 250),
  ('level_master', 'Level Master', 'Completed all levels in a language', '👑', 500)
